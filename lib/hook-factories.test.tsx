...
describe("HookFactories", () => {
    ...
    test("useCompoundState with different names in two components set the same value", () => {
        let valueBumper;
        let valueBumperSecond;
        const TestComponent = () => {
            const [value,setValue] = useCompoundState("first");
            valueBumper = setValue;
            return (<div data-testid="component-with-hook">Value {value}</div>)
        }
        const TestComponentTwo = () => {
            const [value, setValue] = useCompoundState("second");// This default is ignored
        ...
        expect(screen.getByTestId("component-with-hook")).toHaveTextContent(`Value 2`);
        expect(screen.getByTestId("component-with-hook-two")).toHaveTextContent(`Value`);
